...
function StealerKitten:_ready()
	...
	self.Particles = {}
	self.Tweens = {}
	self.Target = self:get_parent():get_node("Kitten") --self.__owner:get_parent():get_node("Kitten")
end
		...
		if self.Target then
			...
			if self.Think and self.Randomizer then
				...
				if Distance > 200 then
					...
					local new = GD.load("res://Objects/Miscelaneous/Particles/ElectricAttackerParticles.tscn"):instance()
					WaitTime = 0.5
					...
					if GUI:get_node("Health") then
						...
						t:interpolate_property(GUI:get_node("Health"),"value",GUI:get_node("Health"):get_as_ratio()*100,(GUI:get_node("Health"):get_as_ratio()-0.05)*100,1,0,3,0)
						self:add_child(t)
						t:start()
