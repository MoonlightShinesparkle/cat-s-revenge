		...
		if self.Target then
			...
			if self.Think and self.Randomizer then
				...
				if Distance > 200 then
					...
					local new = GD.load("res://Objects/Miscelaneous/Particles/FireAttackerParticles.tscn"):instance()
					new:set_position(self.Target:get_position())
					self:get_parent():add_child(new)
					new:set_emitting(true)
					new:get_node("Smoke"):set_emitting(true)
					...
					if GUI:get_node("Health") then
						...
						t:interpolate_property(GUI:get_node("Health"),"value",GUI:get_node("Health"):get_as_ratio()*100,(GUI:get_node("Health"):get_as_ratio()-0.2)*100,1,0,3,0)
						self:add_child(t)
						t:start()
				...
				self.Randomizer:randomize()
				local WaitTime = self.Randomizer:randi_range(1,3)
